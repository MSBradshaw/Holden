...
dataImp <- read_csv('Holden/Analysis-Scripts/Generic-Analysis/imputation-cna-results/imputation-cna-results-4.csv')
for(i in seq(5,50)){
  print(i)
...
dataImp = dataImp[dataImp$Learner != 'MLP',]
dataImp = dataImp[dataImp$Learner != 'SVC',]
...
#read in resampling data
dataRan <- read_csv('Holden/Analysis-Scripts/Generic-Analysis/random-cna-results/random-cna-results-1.csv')
for(i in seq(2:50)){
...
#remove the MLP and SVC from the results because they are crappy
dataRan = dataRan[dataRan$Learner != 'MLP',]
dataRan = dataRan[dataRan$Learner != 'SVC',]
...
#read in resampling data
dataRes <- read_csv('Holden/Analysis-Scripts/Generic-Analysis/cna-resampling-results/resample-cna-1.csv')
for(i in seq(2:50)){
...
#remove the MLP and SVC from the results because they are crappy
dataRes = dataRes[dataRes$Learner != 'MLP',]
dataRes = dataRes[dataRes$Learner != 'SVC',]
...
p4
ggsave(plot = p4,'Holden/Analysis-Scripts/Generic-Analysis/all-results.png',width = 10, height = 7.5, units = c("in"))
