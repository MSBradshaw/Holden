...
data <- read_csv('Holden/Analysis-Scripts/Generic-Analysis/imputation-cna-50-ff/imputation-cna-results-ff-4.csv')
for(i in seq(5,50)){
...
p <- ggplot(data, aes(x=learner, y=score, fill=learner)) + 
  ...
  ggtitle('Imputation CNA Results Full Features') + 
  ylab('Accuracy') +
  xlab('Learner') + ylim(0.0,1.0)
p
ggsave('Holden/Analysis-Scripts/Generic-Analysis/cna-imputation-box-plots-ff.png',width = 10, height = 10, units = c("in"))
