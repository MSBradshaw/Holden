...
data <- read_csv('Holden/Analysis-Scripts/Generic-Analysis/resampling-trans-results/resample-trans-1.csv')
for(i in seq(2:100)){
...
p <- ggplot(data, aes(x=learner, y=score, fill=learner)) + 
  ...
  ggtitle('Resampling Transcriptomics Results') + 
  ylab('Accuracy') +
  xlab('Learner') + ylim(0.0,1.0)
p
ggsave('Holden/Analysis-Scripts/Generic-Analysis/trans-resampling-box-plots.png',width = 10, height = 10, units = c("in"))
