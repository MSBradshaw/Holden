...
ggsave('Chi-Fisher-Changes.png')
ggplot(data=d,aes(x=Size,y=Ratio,color=Test)) + geom_line() + ylab('Ratio: Changes Required for Significance / Size') + 
  theme(text = element_text(size=20)) 
ggsave('Chi-Fisher-Ratio.png')



# This analysis answers the question of how much does it take to fake significance 
# But how much fabrication does it take to make a genotype (group) appear to have the desired phenotype (outcome)?
