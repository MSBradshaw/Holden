...
write_csv(train,'Data/Distribution-Data-Set/train_proteomics_distribution.csv')
write_csv(test,'Data/Distribution-Data-Set/test_proteomics_distribution.csv')
...
plot <- ggplot(data = pca_tibble, aes(x=pc1,y=pc2,colour=label)) +
  geom_point() +
  ggtitle('PCA Resampling Proteomics') +
  xlab(paste('PC1',summary_pca$importance[2,1]) ) +
  ylab(paste('PC2',summary_pca$importance[2,2]) )
plot
ggsave('Analysis-Scripts/Distribution-Analysis/pca-resampling-proteomics.png',plot)
